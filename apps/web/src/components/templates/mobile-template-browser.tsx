...
const STOCK_CATEGORIES = [
	...
	},
	{
		...
		images: [pexelsImg(8386440), pexelsImg(8386434), pexelsImg(2599244)],
	},
	{
		name: "Nature",
...
export function MobileTemplateBrowser({
	...
	const allTemplates = useMemo(() => {
		return categories.flatMap((c) =>
			c.templates.map((t) => ({ ...t, categoryName: c.name })),
		);
	}, [categories]);
	...
	const filteredTemplates = useMemo(() => {
		const templates = activeCategoryId
			? categories
					...
					?.templates.map((t) => ({
						...t,
						categoryName: categories.find((c) => c.id === activeCategoryId)
							?.name,
					})) || []
			: allTemplates;
	...
	const handleUsePexelsVideo = async (video: PexelsVideo) => {
		...
		try {
			...
			const preset = canvasPresets.reduce((prev, curr) => {
				return Math.abs(curr.aspectRatio - videoAspectRatio) <
					Math.abs(prev.aspectRatio - videoAspectRatio)
					? curr
					: prev;
			});
			...
			if (onNavigateToEditor) {
				recordPexelsFlowStep(startedAt, "navigate-to-editor-callback", {
					mediaId,
				});
				onNavigateToEditor();
			} else {
