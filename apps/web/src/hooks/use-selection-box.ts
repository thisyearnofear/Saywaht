...
export function useSelectionBox({
  ...
  // Function to select clips within the selection box
  const selectClipsInBox = useCallback(
    (startPos: { x: number; y: number }, endPos: { x: number; y: number }) => {
      ...
      // Find all timeline clips within the selection rectangle
      const timelineClips = container.querySelectorAll(".timeline-clip");

      const selectedClips: { trackId: string; clipId: string }[] = [];

      timelineClips.forEach((clipElement) => {
        ...
        const trackId = clipElement.getAttribute("data-track-id");

        // Check if clip intersects with selection rectangle (any overlap)
        // Using absolute coordinates for more precise detection
        const intersects = !(
          clipAbsolute.right < normalizedSelection.left ||
          clipAbsolute.left > normalizedSelection.right ||
          clipAbsolute.bottom < normalizedSelection.top ||
          clipAbsolute.top > normalizedSelection.bottom
        );

        if (intersects && clipId && trackId) {
          selectedClips.push({ trackId, clipId });
        }
      });

      // Always call the callback - with clips or empty array to clear selection
      console.log(
        JSON.stringify({ selectClipsInBox: selectedClips.length })
      );
      onSelectionComplete(selectedClips);
    },
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      ...
      // Don't call selectClipsInBox again - real-time selection already handled it
