...
export function FarcasterClientLogic() {
  ...
  const { initializeFarcaster, setFrameState, isFarcasterMiniApp } = useFarcasterContext();

  // Initialize Farcaster context with frame data
  useEffect(() => {
    ...
    initFarcaster();
  }, [searchParams, setFrameState, initializeFarcaster]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !isFarcasterMiniApp) {
      ...
      const isFarcaster = window.location.search.includes('farcaster') || 
